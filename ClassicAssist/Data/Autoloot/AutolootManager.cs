...
namespace ClassicAssist.Data.Autoloot
{
    public class AutolootManager
    {
        ...
        // Snapshot of entry names for binding from XAML (e.g. AutolootValueControl's Match editor).
        // x:Static evaluates the getter each time a row template is loaded, so newly added entries
        // appear on subsequent row creations — matching the original code-behind's one-shot snapshot.
        public static IEnumerable<string> AutolootEntryNames =>
            GetInstance().GetEntries().Select( ale => ale.Name ).ToArray();
