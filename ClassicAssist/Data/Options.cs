...
namespace ClassicAssist.Data
{
    public class Options : SetPropertyNotifyChanged
    {
        static Options()
        {
            // Avalonia.Media.Color has no built-in TypeConverter that Newtonsoft can use;
            // profile files written by the WPF build store colors as "#AARRGGBB" strings.
            // Register globally so every json["X"]?.ToObject<...>() call across the VMs
            // picks it up.
            Newtonsoft.Json.JsonConvert.DefaultSettings = () =>
            {
                var s = new Newtonsoft.Json.JsonSerializerSettings();
                s.Converters.Add( new ClassicAssist.Misc.AvaloniaColorJsonConverter() );
                return s;
            };
        }
        ...
        private Avalonia.Controls.GridLength _chatWindowRightColumn;
        ...
        public Avalonia.Controls.GridLength ChatWindowRightColumn
        {
        ...
        public static void Save( Options options )
        {
            lock ( _serializeLock )
            {
                ...
                // Per-instance try/catch: one VM throwing (e.g. a Serialize* NRE on a
                // partially-initialized collection) must not abort the entire profile save.
                // Without this, a single broken VM means the user's profile changes are lost
                // on shutdown — and shutdown runs from OnClientClosing across the CLR boundary,
                // where exceptions are extra costly.
                foreach ( BaseViewModel instance in instances )
                {
                    if ( instance is ISettingProvider settingProvider )
                    {
                        try { settingProvider.Serialize( obj ); }
                        catch ( Exception ex ) { Console.Error.WriteLine( $"[ClassicAssist] Options.Save: {instance.GetType().Name}.Serialize failed: {ex.GetType().Name}: {ex.Message}" ); }
                    }
                    ...
                    {
                        Console.Error.WriteLine( $"[ClassicAssist] Options.Save: {instance.GetType().Name}.Serialize(global) failed: {ex.GetType().Name}: {ex.Message}" );
                    }
                }
        ...
        // Cache the last successfully loaded profile JSON so that ViewModels constructed
        // AFTER Load returns (e.g. when a TabControl lazily realizes a tab the user clicks)
        // can hydrate themselves on demand via HydrateIfLoaded. Without this they'd start
        // empty and the corresponding tab would show no data.
        private static JObject _lastLoadedJson;

        public static void HydrateIfLoaded( BaseViewModel instance )
        {
            if ( _lastLoadedJson == null || instance == null )
            {
                return;
            }
            ...
            try
            {
                if ( instance is ISettingProvider settingProvider )
                {
                    settingProvider.Deserialize( _lastLoadedJson, Options.CurrentOptions );
                }
                if ( instance is IGlobalSettingProvider globalSettingProvider )
                {
                    ...
                    if ( File.Exists( filePath ) )
                    {
                        ...
                        globalSettingProvider.Deserialize( global, Options.CurrentOptions, true );
                    }
                }
            }
            catch ( Exception ex )
            {
                Console.WriteLine( $"[Options.HydrateIfLoaded] {instance.GetType().Name} failed: {ex.Message}" );
            }
        }
        ...
        public static void Load( string optionsFile, Options options )
        {
            lock ( _serializeLock )
            {
                try
                {
                    ...
                    foreach ( BaseViewModel instance in instances )
                    {
                        // Per-VM try/catch: one VM's Deserialize blowing up (e.g. missing
                        // resource string or partial Avalonia port) must not abort the
                        // remaining VMs' load. Without this, a single bad VM left the
                        // entire tab strip empty.
                        try
                        {
                            if ( instance is ISettingProvider settingProvider )
                            {
                        ...
                        {
                            Console.WriteLine( $"[Options.Load] {instance.GetType().Name}.Deserialize failed: {( vmEx.InnerException ?? vmEx )}" );
                        }
                    }
                }
                catch ( Exception e )
                {
                    // Removed DEBUG-only MessageBox+Environment.Exit(0): MessageBox is a
                    // no-op shim under Avalonia, and Exit(0) silently kills the process
                    // making the failure look like a window flash. Always log loudly.
                    Console.WriteLine( $"[Options.Load] Profile '{optionsFile}' failed: {e}" );
