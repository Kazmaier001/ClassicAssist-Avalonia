...
namespace ClassicAssist.UI.Controls
{
    ...
    /// </summary>
    public partial class HexDumpControl : UserControl, INotifyPropertyChanged
    {
        public static readonly StyledProperty<PacketEntry> PacketProperty = AvaloniaProperty.Register<HexDumpControl, PacketEntry>( nameof( Packet ) );

        public static readonly StyledProperty<ObservableCollection<DebugPacketsViewModel.PacketEnabledEntry>> PacketEntriesProperty = AvaloniaProperty.Register<HexDumpControl, ObservableCollection<DebugPacketsViewModel.PacketEnabledEntry>>( nameof( PacketEntries ) );

        private string _binaryData;
        private string _textData;

        static HexDumpControl()
        {
            // Avalonia StyledProperty changes don't run the C# property setter when set
            // via XAML binding; wire the change callback explicitly so BinaryData/TextData
            // actually get computed. (See memory: avalonia-property-porting.)
            PacketProperty.Changed.AddClassHandler<HexDumpControl>( ( ctrl, e ) =>
            {
                PacketEntry entry = e.NewValue as PacketEntry;
                ctrl.BinaryData = SetBinary( entry );
                ctrl.TextData = SetText( entry );
                ctrl.OnPropertyChanged( nameof( Status ) );
            } );
        }
        ...
        private void Copy_OnClick( object sender, RoutedEventArgs e )
        {
            try
            {
                ...
                ClipboardCompat.SetText( prepend );
            }
            catch ( Exception )
            {
                // ignored
