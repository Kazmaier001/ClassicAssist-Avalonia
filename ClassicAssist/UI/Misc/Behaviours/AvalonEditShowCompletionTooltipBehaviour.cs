...
namespace ClassicAssist.UI.Misc.Behaviours
{
    ...
    {
        public static readonly StyledProperty<Dictionary<string, object>> FrameVariablesProperty = AvaloniaProperty.Register<AvalonEditShowCompletionTooltipBehaviour, Dictionary<string, object>>( nameof( FrameVariables ) );

        public static readonly StyledProperty<bool> IsPausedProperty = AvaloniaProperty.Register<AvalonEditShowCompletionTooltipBehaviour, bool>( nameof( IsPaused ) );
        ...
        protected override void OnAttached()
        {
            ...
            _textEditor.PointerEntered += OnMouseHover;
            _textEditor.PointerExited += OnMouseHoverStopped;
        }
        ...
        private void OnMouseHover( object sender, PointerEventArgs e )
        {
            ...
            try
            {
                ...
                if ( matches.Any() )
                {
                    ...
                    {
                        TextBlock descriptionText = new TextBlock { Text = match.Description.ToString(), TextWrapping = Avalonia.Media.TextWrapping.Wrap, Margin = new Thickness( 0, 2, 0, 5 ) };
                        panel.Children.Add( descriptionText );
                        TextBlock nameText = new TextBlock { Text = match.Name, FontWeight = FontWeight.Bold, Margin = new Thickness( 0, 5, 0, 0 ) };
                        panel.Children.Add( nameText );
                    }

                // PlacementTarget removed (use Avalonia ToolTip attached properties)
                    _toolTip.Content = panel;
                    ToolTip.SetIsOpen( _toolTip, true );
                }
                else if ( IsPaused )
                {
                    ...
                    if ( frameVariables.Any() )
                    {
                        ...
                        foreach ( KeyValuePair<string, object> variable in frameVariables )
                        {
                            ...
                            {
                                Text = $"{variable.Key} : {variable.Value?.GetType().Name ?? "null"}", FontWeight = FontWeight.Bold, Margin = new Thickness( 0, 5, 0, 0 )
                            };
                            panel.Children.Add( nameText );
                            TextBlock valueText = new TextBlock
                            {
                                Text = MacroInvoker.GetDisplayValue( variable.Value ), TextWrapping = Avalonia.Media.TextWrapping.Wrap, Margin = new Thickness( 0, 2, 0, 5 )
                            };
        ...
        private void OnTextEntered( object sender, TextInputEventArgs e )
        {
            ...
            {
                CloseWhenCaretAtBeginning = true, CloseAutomatically = false, Width = 500
            };
