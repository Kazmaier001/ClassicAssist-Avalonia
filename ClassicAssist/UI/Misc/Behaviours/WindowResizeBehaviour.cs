...
#if !DEVELOP
            // Only restore stored size if a profile has actually populated these; otherwise
            // leave the AXAML-declared Width/Height in place (the static defaults are 0).
            if ( AssistantOptions.WindowWidth > 0 )
                AssociatedObject.Width = AssistantOptions.WindowWidth;
            if ( AssistantOptions.WindowHeight > 0 )
