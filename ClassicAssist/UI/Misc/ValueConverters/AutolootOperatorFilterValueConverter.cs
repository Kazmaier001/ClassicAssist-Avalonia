...
namespace ClassicAssist.UI.Misc.ValueConverters
{
...

    {
        ...
        {
            if ( values.Count < 2 || !( values[0] is Type enumType ) )
            {
                return null;
            }
            ...
            if ( !(values[1] is PropertyEntry entry) || entry.AllowedOperators == AutolootAllowedOperators.All || entry.AllowedOperators == 0 )
            {
                return allValues;
            }
