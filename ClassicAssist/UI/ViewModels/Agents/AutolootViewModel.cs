...
namespace ClassicAssist.UI.ViewModels.Agents
{
    public class AutolootViewModel : BaseViewModel, ISettingProvider
    {
        ...
        private readonly AutolootManager _manager;
        ...
        public ICommand InsertConstraintCommand => _insertConstraintCommand ?? ( _insertConstraintCommand = new RelayCommand( InsertConstraint, o => SelectedItem != null ) );
        ...
        public ICommand RemoveConstraintCommand => _removeConstraintCommand ?? ( _removeConstraintCommand = new RelayCommand( RemoveConstraint, o => SelectedProperty != null ) );
        ...
        {
            get => _selectedItem;
            set => SetProperty( ref _selectedItem, value, afterChange: _ =>
            {
                // Avalonia doesn't auto-requery RelayCommand on bound-property changes.
                // Commands whose CanExecute reads SelectedItem need an explicit nudge.
                ( _insertConstraintCommand as RelayCommand )?.RaiseCanExecuteChanged();
                ( _selectHueCommand as RelayCommand )?.RaiseCanExecuteChanged();
            } );
        }
        ...
        public AutolootConstraintEntry SelectedProperty
        {
            ...
            set => SetProperty( ref _selectedProperty, value, afterChange: _ =>
            {
                // Same Avalonia requery nudge as SelectedItem above.
                ( _removeConstraintCommand as RelayCommand )?.RaiseCanExecuteChanged();
            } );
        }
        ...
        {
            string text = ClipboardCompat.GetText();
            ...
            lock ( _autolootLock )
            {
                try
                {
                    ...
                    if ( Engine.TooltipsEnabled )
                    {
                        ...
                        stopWatch.Start();
#endif
                        bool result = UOC.WaitForPropertiesAsync( items.Where( e => e != null && e.Properties == null ), 1000 ).Result;
        ...
        private void InsertConstraint( object obj )
        {
            ...
            List<AutolootConstraintEntry> constraints = new List<AutolootConstraintEntry>( SelectedItem.Constraints ) { new AutolootConstraintEntry { Property = propertyEntry } };
        ...
        {
            AutolootEntry entry = new AutolootEntry { Name = Strings.Any, ID = -1, Constraints = new ObservableCollection<AutolootConstraintEntry>() };
        ...
        private async Task Remove( object arg )
        {
            ...
            // Clear SelectedItem before removing so the right-side editor doesn't keep
            // pointing at the just-deleted entry when Draggables ends up empty
            // (SetNewSelectedIndex catches and ignores the index-out-of-bounds case).
        ...
        private static void SelectHue( object obj )
        {
            ...
            if ( HuePickerWindow.GetHue( entry.RehueHue, out int hue ) )
            {
                entry.RehueHue = hue;
            }
        }
