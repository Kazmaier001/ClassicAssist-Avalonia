...
namespace ClassicAssist.UI.ViewModels.Agents
{
    public class CountersTabViewModel : BaseViewModel, ISettingProvider
    {
        ...
        public CountersTabViewModel()
        {
            ...
            // RecountCommand.CanExecute reads Items.Count > 0; without a collection
            // listener it's only evaluated at command construction time (when Items
            // is empty) and never re-checked when entries get added/removed.
            Items.CollectionChanged += ( s, e ) =>
                ( _recountCommand as RelayCommand )?.RaiseCanExecuteChanged();
        }
        ...
        public CountersAgentEntry SelectedItem
        {
            ...
            set
            {
                ...
                ( _removeEntryCommand as RelayCommand )?.RaiseCanExecuteChanged();
            }
        }
