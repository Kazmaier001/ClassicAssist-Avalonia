    ...
    public class FriendsTabViewModel : BaseViewModel, ISettingProvider
    {
        ...
        public FriendEntry SelectedItem
        {
            ...
            set
            {
                ...
                ( _removeFriendCommand as RelayCommandAsync )?.RaiseCanExecuteChanged();
            }
        }

        public ICommand SelectHueCommand =>
            _selectHueCommand ?? ( _selectHueCommand = new RelayCommand( SelectHue ) );

        public void Serialize( JObject json, bool global = false )
        {
            // Options is set in Deserialize; null when no profile has been loaded.
            if ( Options == null )
                return;
        ...
        {
            if ( !HuePickerWindow.GetHue( Options.CurrentOptions.RehueFriendsHue, out int hue ) )
            {
                return;
            }
