...
namespace ClassicAssist.UI.ViewModels
{
    public class ChatViewModel : BaseViewModel
    {
        ...
        private Avalonia.Controls.GridLength _rightColumnSize = new Avalonia.Controls.GridLength( 220 );
        ...
        public ICommand SendMessageCommand => _sendMessageCommand ?? ( _sendMessageCommand = new RelayCommand( SendMessage, o => true ) );
        ...
        public Avalonia.Controls.GridLength RightColumnSize
        {
        ...
        private void ChangeChannel( object obj )
        {
            ...
            Dispatcher.UIThread.Invoke( () => { Commands.JoinChatChannel( channel ); } );
        }

        private void OnChannelRemoved( string channel )
        {
            Dispatcher.UIThread.Invoke( () => { Channels.Remove( channel ); } );
        }

        private void OnChannelCreated( string channel )
        {
            Dispatcher.UIThread.Invoke( () => { Channels.Add( channel ); } );
        }

        private void OnChatMessage( string username, string channel, string message )
        {
            Dispatcher.UIThread.Invoke( () => Messages.Add( new ChatMessage { Username = username, Channel = channel, Text = message, Colour = GetUserColour( username ) } ) );
        }
        ...
        {
            Dispatcher.UIThread.Invoke( () => Users.Clear() );
        }

        private void OnUserLeft( string username, string channel )
        {
            Dispatcher.UIThread.Invoke( () => { Users.Remove( Users.FirstOrDefault( u => u.Username == username ) ); } );
        }
