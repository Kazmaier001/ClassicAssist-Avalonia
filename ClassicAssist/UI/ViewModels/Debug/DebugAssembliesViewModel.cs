...
namespace ClassicAssist.UI.ViewModels.Debug
{
    public class DebugAssembliesViewModel : BaseViewModel
    {
        ...
        private async Task Load( object arg )
        {
            ...
            if ( result.HasValue && result.Value )
            {
                try
                {
                    ...
                    if ( assembly != null )
                    {
                        await Dispatcher.UIThread.InvokeAsync( () => Items.Add( assembly ) );
                    }
                }
                catch ( Exception e )
                {
                    Console.WriteLine( string.Format( Strings.Error_loading_assembly___0_, e.Message ) );
                }
            }
        }
        ...
        private async Task Remove( object arg )
        {
            ...
            await Dispatcher.UIThread.InvokeAsync( () => Items.Remove( assembly ) );
        }
