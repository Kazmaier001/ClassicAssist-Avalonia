    ...
    public class EntityCollectionViewerViewModel : BaseViewModel
    {
        ...
        private void AutolootContainer( object param )
        {
            ...
            EnqueueAction( async obj =>
            {
                foreach ( var item in lootItems.Select( ( value, i ) => new { i, value } ) )
                {
                    ...
                    {
                        Dispatcher.UIThread.Invoke( () => { obj.Status = Strings.Cancel; } );
                        return false;
                    }

                    Dispatcher.UIThread.Invoke( () => { obj.Status = string.Format( Strings.Moving_item__0_____1_, item.i, lootItems.Count ); } );
        ...
        private void ContextCustomAction( object arg )
        {
            ...
            EnqueueAction( obj =>
            {
                ...
                {
                    Dispatcher.UIThread.Invoke( () => { obj.Status = $"{action.Key} {item.i} / {items.Count}"; } );
        ...
        private async Task ContextMoveToGround( object arg )
        {
            ...
            EnqueueAction( async obj =>
            {
                foreach ( var item in items.Select( ( value, i ) => new { i, value } ) )
                {
                    ...
                    Dispatcher.UIThread.Invoke( () => { obj.Status = string.Format( Strings.Moving_item__0_____1_, item.i, items.Length ); } );
        ...
        private void ContextOpenContainer( object arg )
        {
            ...
            EnqueueAction( async obj =>
            {
                ...
                {
                    Dispatcher.UIThread.Invoke( () => { obj.Status = string.Format( Strings.Opening_container__0_____1____, item.Index, containerSerials.Length ); } );
        ...
        private void ProcessQueue( QueueAction obj )
        {
            ...
            Dispatcher.UIThread.Invoke( () => { QueueActions.Remove( obj ); } );
        }
        ...
        private void ContextTarget( object obj )
        {
            ...
            EnqueueAction( async queueAction =>
            {
                foreach ( var item in items.Select( ( value, i ) => new { i, value } ) )
                {
                    ...
                    {
                        Dispatcher.UIThread.Invoke( () => { queueAction.Status = Strings.Cancel; } );
                        return false;
                    }

                    Dispatcher.UIThread.Invoke( () => { queueAction.Status = string.Format( Strings.Targeting_item__0_____1_, item.i, items.Count ); } );
        ...
        private void OpenAllContainers( object obj )
        {
            ...
            async Task<List<Item>> OpenContainers( IEnumerable<Item> items, QueueAction queueAction )
            {
                ...
                {
                    Dispatcher.UIThread.Invoke( () => { queueAction.Status = string.Format( Strings.Opening_container__0_____1____, item.index, containers.Count ); } );
                    int pos = 19;
