...
namespace ClassicAssist.UI.ViewModels.Filters
{
    public class ItemIDFilterConfigureWindowViewModel : BaseViewModel
    {
        ...
        private static async void SelectDestinationID( object obj )
        {
            ...
            // ShowDialog() is fire-and-forget; ShowDialogAsync awaits the
            // user's OK/Cancel so SelectedItem / Result are populated by the
            // time we read them.
            await window.ShowDialogAsync();

            if ( window.Result && window.SelectedItem != null )
            {
        ...
        private static void SelectHue( object obj )
        {
            ...
            if ( HuePickerWindow.GetHue( entry.Hue, out int hue ) )
            {
                entry.Hue = hue;
            }
        }

        private static async void SelectSourceID( object obj )
        {
            if ( !( obj is ItemIDFilterEntry entry ) )
            {
                return;
