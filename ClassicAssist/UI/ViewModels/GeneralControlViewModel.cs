...
namespace ClassicAssist.UI.ViewModels
{
    public class GeneralControlViewModel : BaseViewModel, ISettingProvider
    {
        ...
        public bool IsLinkedProfile
        {
            ...
            set
            {
                SetProperty( ref _isLinkedProfile, value );
                OnPropertyChanged( nameof( LinkUnlinkTooltip ) );
            }
        }

        public string LinkUnlinkTooltip =>
            IsLinkedProfile ? Shared.Resources.Strings.Linked : Shared.Resources.Strings.Unlinked;
        ...
        public bool SavePasswords
        {
            ...
            set
            {
                AssistantOptions.SavePasswords = value;
                OnPropertyChanged();
                // Avalonia has no CommandManager.RequerySuggested, so RelayCommand
                // CanExecute predicates don't re-evaluate when their inputs change —
                // we must poke them manually.  Without this the Remove button stays
                // disabled even after the user ticks Enabled.
                ( _removeSavedPasswordCommand as RelayCommand )?.RaiseCanExecuteChanged();
            }
        }
