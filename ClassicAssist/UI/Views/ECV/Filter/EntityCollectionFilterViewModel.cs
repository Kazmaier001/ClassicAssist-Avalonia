    ...
    public class EntityCollectionFilterViewModel : SetPropertyNotifyChanged
    {
        ...
        public EntityCollectionFilterEntry SelectedProfile
        {
            ...
            set
            {
                ...
                // Keep Item in lockstep with the selected profile so the bound UI
                // refreshes regardless of who set SelectedProfile. WPF relied on
                // ComboBoxBehavior firing ChangeProfileCommand to do this, but its
                // user-triggered detection uses PointerPressed on the ComboBox,
                // which doesn't bubble from popup items in Avalonia (separate
                // visual tree). Result was: SelectedProfile changed but Item stayed
                // pointing at the previous profile, so the dropdown looked changed
                // but the data didn't.
                if ( value != null )
                {
                    Item = value;
        ...
        private void Apply( object obj )
        {
            ...
            // Apply is the user's explicit "commit" — persist profiles so they
            // survive a close/reopen of the ECV window. The IsVisible-changed
            // hook in the control isn't reliable on Avalonia: window close
            // doesn't always tick IsVisible to false before the control is gone.
            SaveFilterProfiles();
        }
