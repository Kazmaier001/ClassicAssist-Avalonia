...
namespace ClassicAssist.UI.Views
{
    ...
    public partial class HuePickerWindow : Window, INotifyPropertyChanged
    {
        ...
        public HuePickerWindow() : this( -1 )
        {
        }

        public HuePickerWindow( int initialHue )
        {
            InitializeComponent();
            ...
            if ( initialHue > 0 && initialHue <= Items.Count )
            {
                HuePickerEntry match = FilteredItems.FirstOrDefault( e => e.Index == initialHue );

                if ( match != null )
                {
                    SelectedItem = match;
                    Opened += ( _, _ ) => Dispatcher.UIThread.Post( () => listView?.ScrollIntoView( match ) );
                }
            }
        }
        ...
        {
            return GetHue( -1, out hue );
        }

        public static bool GetHue( int initialHue, out int hue )
        {
            HuePickerWindow window = new HuePickerWindow( initialHue );

            Window parent = Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop
                ? desktop.MainWindow
                : null;
