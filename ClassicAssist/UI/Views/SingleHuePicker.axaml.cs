...
namespace ClassicAssist.UI.Views
{
    ...
    public partial class SingleHuePickerWindow : Window, INotifyPropertyChanged
    {
        ...
        public SingleHuePickerWindow()
        {
            ...
            // Callers set SelectedHue via object-initializer (runs AFTER the ctor), so
            // resolve SelectedItem and scroll into view on Opened, when SelectedHue is final.
            Opened += ( _, _ ) =>
            {
                if ( SelectedHue > 0 )
                {
                    SelectedItem = FilteredItems.FirstOrDefault( i => i.Index == SelectedHue );
                }
