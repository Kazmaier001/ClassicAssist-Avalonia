...
namespace ClassicAssist.UO
{
    public class Commands
    {
        ...
        public static void SystemMessage( string text, int hue = 0x03b2, bool suppressInQuietMode = false,
            bool throttleRepeating = false )
        {
            // Suppress when not in-game. The 0xAE packet goes via CUO's Recv fnptr into the
            // client's incoming-packet handler, which dereferences World.Player / Mobiles /
            // etc. When the user is on the login screen those are null and CUO faults — and
            // because the call crosses the CoreCLR↔net472 boundary the fault hangs the game
            // window and triggers the Windows error reporter. Mirror WPF Razor/Assistant
            // practice: only send chat-channel packets when connected.
            if ( !Engine.Connected )
                return;
        ...
        public static async Task InspectObjectAsync()
        {
            ...
            if ( targetType == TargetType.Object && serial != 0 )
            {
                ...
                // Avalonia Window ctors VerifyAccess() and throw off the UI thread —
                // the WPF idiom of an STA thread per modal dialog is broken here.
                // Marshal to the Avalonia UI thread instead. See [[cuo-plugin-loading]].
                Dispatcher.UIThread.Post( () =>
                {
                    ...
                    catch ( Exception ex ) { Console.Error.WriteLine( $"InspectObject(entity): {ex}" ); }
                } );
            }
            else
            {
                if ( itemID == 0 )
                {
                    ...
                    Dispatcher.UIThread.Post( () =>
                    {
                        ...
                        catch ( Exception ex ) { Console.Error.WriteLine( $"InspectObject(landTile): {ex}" ); }
                    } );
                }
                else
                {
                    ...
                    Dispatcher.UIThread.Post( () =>
                    {
                        ...
                        catch ( Exception ex ) { Console.Error.WriteLine( $"InspectObject(static): {ex}" ); }
                    } );
                }
            }
        }
