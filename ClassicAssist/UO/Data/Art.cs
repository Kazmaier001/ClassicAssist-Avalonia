...
namespace ClassicAssist.UO.Data
{
    public static class Art
    {
        ...
        public static SKBitmap GetStatic( int itemID, int hue )
        {
            SKBitmap bmp = GetStatic( itemID );
        ...
        public static unsafe SKBitmap GetStatic( int itemId )
        {
            itemId += 0x4000;
            ...
            using ( BinaryReader reader = new BinaryReader( artFile ) )
            {
                ...
                SKBitmap bmp = new SKBitmap( new SKImageInfo( width, height, SKColorType.Bgra8888, SKAlphaType.Unpremul ) );
                // SKBitmap memory is zero-initialised, so untouched pixels stay transparent (0x00000000) -- matches 1555 zero-pixel semantics.

                int rowPixels = bmp.RowBytes >> 2;
                uint* line = (uint*) bmp.GetPixels().ToPointer();

                for ( int y = 0; y < height; y++, line += rowPixels )
                {
                    ...
                    while ( ( xOffset = reader.ReadUInt16() ) + ( xRun = reader.ReadUInt16() ) != 0 )
                    {
                        ...
                        while ( cur < end )
                        {
                            // Same XOR 0x8000 alpha-bit flip the legacy reader does; we then convert to BGRA.
                            ushort c1555 = (ushort) ( reader.ReadUInt16() ^ 0x8000 );
                            *cur++ = HueEntry.Convert1555ToBgra8888( c1555 );
                        }
                    }
                }
