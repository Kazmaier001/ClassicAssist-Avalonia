    ...
    public static class Skills
    {
        ...
        internal static Dictionary<int, SkillData> LoadSkills()
        {
            ...
            for ( int x = 0; x < indexBytes.Length / 12; x++ )
            {
                ...
                // skills.mul entry: [1 byte invokable] + [name ASCII bytes].
                // The WPF source did `length - 2`, which was an off-by-one —
                // every skill name lost its trailing character. Some shards
                // emit a null terminator, others don't, so read `length - 1`
                // bytes and TrimEnd to handle both formats.
                skills.Add( x,
                    new SkillData
                    {
                        Invokable = mulBytes[start] == 1,
                        Name = Encoding.ASCII
                            .GetString( mulBytes, start + 1, length - 1 )
                            .TrimEnd( '\0', ' ', '\r', '\n' )
                    } );
            }
