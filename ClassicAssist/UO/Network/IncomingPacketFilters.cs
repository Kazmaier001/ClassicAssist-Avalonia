...
namespace ClassicAssist.UO.Network
{
    public static class IncomingPacketFilters
    {
        ...
        // 0x88 OpenPaperdoll layout: [0]=id, [1..4]=serial, [5..64]=60-byte ASCII name,
        // [65]=flags. NameOverride support — overwrites the paperdoll title-bar name
        // in place (no resize) so the override surfaces when double-clicking a mobile.
        private static bool OnOpenPaperdoll( ref byte[] packet, ref int length )
        {
            ...
            Array.Resize( ref nameOverrideBytes, 60 );
            Array.Copy( nameOverrideBytes, 0, packet, 5, 60 );
